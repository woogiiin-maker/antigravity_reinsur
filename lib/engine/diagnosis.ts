...
/**
 * 연령대별 표준 가이드라인 권장 설계 전략 및 기준 산출 (태아~시니어 전 생애주기)
 */
export function getAgeGroupStrategy(age: number, isFetus?: boolean): AgeGroupStrategyInfo {
  if (isFetus || age < 0) {
    return {
      ...
      strategyDesc:
        '출생 직후 발생 가능한 선천성 기형(Q코드 다빈도 혀유착/이개전우공 및 심장/뇌 기형) 수술비와 미숙아 인큐베이터 입원일당 확보가 최우선입니다. 100세 만기로 무리하게 10만원 이상 지출하기보다, 30세 만기로 3~5만원대 슬림하게 필수 특약을 채우고 30세 시점에 무심사 100세 계약전환하는 것을 권장합니다.',
      sources: '설계 표준 가이드라인',
      keyPoints: [
  ...
  if (age < 30) {
    return {
      ...
      strategyDesc:
        '보험료가 가장 저렴한 20대 골든타임입니다. 해약환급금 미지급형(무해지)으로 보험료를 20~30% 절감하면서, 나이가 들면 축소되는 뇌혈관·허혈성 진단비를 3,000만원까지 확보하고 일반암 5,000만원을 20년납 90세만기 비갱신형으로 구축합니다.',
      sources: '설계 표준 가이드라인',
      keyPoints: [
        '해약환급금 미지급형(무해지) 선택으로 표준형 대비 20~30% 저렴하게 가입',
        '뇌혈관·허혈성 진단비 3,000만원 축소 전 선점',
        '일반암 5,000만원 20년납 90세만기 비갱신형 완성',
      ],
    };
  }
  if (age < 50) {
    return {
      ...
      strategyDesc:
        '가계 지출과 자녀 양육 부담이 가장 큰 시기입니다. 중대 질환 발병 시 최소 1~2년 치 연봉 수준(5,000만원)의 일반암 진단비를 확보하여 치료 기간의 생활비를 방어하고, 최신 표적·중입자 치료를 대비하는 종합 비급여 암치료비를 결합하는 것이 정석입니다.',
      sources: '설계 표준 가이드라인',
      keyPoints: [
        '최소 1~2년 치 연봉 수준(5,000만원) 일반암 진단비로 소득 공백 방어',
        '종합 비급여 암주요치료비 및 생활비 결합',
  ...
  if (age < 60) {
    return {
      ...
      strategyDesc:
        '암·뇌·심장 발병률이 급격히 치솟는 위험 구간입니다. 진단비 보험료 부담을 고려하여 일반암 5,000만원 수준을 방어선으로 두고, 최신 표적·중입자 치료를 보장하는 비급여 주요치료비와 다빈도 1~5종 수술비, 혈관질환 산정특례를 탄탄히 보강합니다.',
      sources: '설계 표준 가이드라인',
      keyPoints: [
        '비급여 치료비와 종수술비 조합으로 가성비 최적화',
  ...
  return {
    ...
    strategyDesc:
      '보험사 인수 한도가 줄고 진단비 보험료가 매우 비싼 시기입니다. 무리한 고액 진단비보다 적은 보험료로 고액 비급여 치료를 해결하는 비급여암 주요치료비와 하루 15~20만원 지급되는 간병인 사용일당, 다빈도 수술비 위주로 실속 있게 구성합니다.',
    sources: '설계 표준 가이드라인',
    keyPoints: [
      '비싼 진단비 지양, 하루 20만원 간병인 사용일당 최우선 확보',
...
export function calculateRecommendedCoverages(profile: UserProfile): Record<CoverageKey, number> {
  ...
  if (isFetus || age <= 18) {
    ...
    base = {
      ...
      nonReimbursedCancer: 30_000_000,
      ...
      injuryDisability: 200_000_000,
      diseaseDisability80: 50_000_000,
      injurySurgery: 1_000_000,
      diseaseSurgery: 2_000_000,
      circulatoryCare: 20_000_000,
      surgery: 10_000_000,
    ...
    // 30~40대 (경제활동·가장 플랜: 표의 권장 최대금액 기준 100% 충족 세팅)
    base = {
      cancer: 50_000_000,
      similarCancer: 10_000_000,
      nonReimbursedCancer: 30_000_000,
  ...
  const scored = eligible.map((prod) => {
    ...
    if (isFetus) {
      ...
      if (prod.insurerName === '현대해상') {
        matchScore += 20;
        reasons.push('업계 선호도 1위 (대한민국 태아보험 부동의 점유율)');
      }
